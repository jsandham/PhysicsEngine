...
// dt2^2
float ClosestDistance::closestDistance(Ray ray1, Ray ray2)
{
	...
	if (std::abs(det) > FLT_MIN) {
		...
		float t1 = invDet * (v2v2 * dpv1 - v1v2 * dpv2);
		float t2 = invDet * (v1v2 * dpv1 - v1v1 * dpv2);
...
float ClosestDistance::closestDistance(Ray ray, Circle circle)
{
	...
	//float D = glm::dot(circle.mNormal, circle.mOrigin);

	//float t = (D - glm::dot(circle.mNormal, ray.mOrigin)) / glm::dot(circle.mNormal, ray.mDirection);

	//if (t < 0.0f) {
	//	return glm::distance(ray.mOrigin, circle.mOrigin);
	//}

	//float distance = glm::distance(ray.getPoint(t), circle.mOrigin);

	//if (distance >= circle.mRadius) {
	//	distance = distance - circle.mRadius;
	//}
	//else {
	//	distance = circle.mRadius - distance;
	//}
