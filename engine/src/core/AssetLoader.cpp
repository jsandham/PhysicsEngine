...
bool AssetLoader::load(const std::string& filepath, Material& material)
{
	...
	json::JSON::JSONWrapper<map<string, JSON>> objects = jsonMaterial.ObjectRange();
	...
	for (it = objects.begin(); it != objects.end(); it++) {
		...
		if (it->first == "shader") {
			material.shaderId = Guid(it->second.ToString());
		...
		uniform.isEditorExposed = it->second["isEditorExposed"].ToBool();
