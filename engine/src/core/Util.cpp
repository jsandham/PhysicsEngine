...
void Util::poissonSampler(std::vector<float> &points, float minx, float miny, float minz, float maxx, float maxy, float maxz, float h, float r, unsigned int k)
{
	std::vector<int> grid;
...
void Util::randomParticles(std::vector<float> &points, float minx, float miny, float minz, float maxx, float maxy, float maxz, float h, int numPoints)
{
	std::default_random_engine generator;
...
void Util::gridOfParticlesXYPlane(std::vector<float> &points, float dx, float dy, float z, int nx, int ny)
{
	for (int i = 0; i < nx; i++){
		for (int j = 0; j < ny; j++){
...
void Util::gridOfParticlesXZPlane(std::vector<float> &points, float dx, float dz, float y, int nx, int nz)
{
	for (int i = 0; i < nx; i++){
		for (int j = 0; j < nz; j++){
			points.push_back(i*dx);
...
int Util::findGridLocation(float x, float y, float z, float minx, float miny, float minz, int nx, int ny, float dr)
{
	return ny * nx * (int)floor((z - minz) / dr) + nx * (int)floor((y - miny) / dr) + (int)floor((x - minx) / dr);
}

bool Util::IsNearAnotherPoint(std::vector<int> &grid, std::vector<float> &points, float x, float y, float z, int index, int nx, int ny, int nz, float r)
{
	for (int a = -2; a <= 2; a++){
		for (int b = -2; b <= 2; b++){
			for (int c = -2; c <= 2; c++){
...
//void Util::poissonSampler(std::vector<glm::vec3> &points, glm::vec3 min, glm::vec3 max, float h, float r, unsigned int k)
//{
//	std::vector<int> grid;
...
//
//
//int Util::findGridLocation(glm::vec3 point, glm::vec3 min, int nx, int ny, float dr)
//{
//	return ny * nx * (int)floor((point.z- min.z) / dr) + nx * (int)floor((point.y - min.y) / dr) + (int)floor((point.x - min.x) / dr);
//}
//
//bool Util::IsNearAnotherPoint(std::vector<int> &grid, std::vector<glm::vec3> &points, glm::vec3 point, int index, int nx, int ny, int nz, float r)
//{
//	for (int a = -2; a <= 2; a++){
//		for (int b = -2; b <= 2; b++){
//			for (int c = -2; c <= 2; c++){
