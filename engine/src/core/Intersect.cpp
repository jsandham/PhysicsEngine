...
{
    /*glm::vec3 invD = 1.0f / ray.mDirection;
    glm::vec3 t0s = (bmin - ray.mOrigin) * invD;
    glm::vec3 t1s = (bmax - ray.mOrigin) * invD;

    glm::vec3 tsmaller = glm::min(t0s, t1s);
    glm::vec3 tbigger = glm::max(t0s, t1s);

    float tmin = glm::max(0.0f, glm::max(tsmaller[0], glm::max(tsmaller[1], tsmaller[2])));
    float tmax = glm::min(1e30f, glm::min(tbigger[0], glm::min(tbigger[1], tbigger[2])));
