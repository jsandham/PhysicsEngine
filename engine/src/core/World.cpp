...
bool World::loadAsset(const std::string &filePath)
{
    ...
    PhysicsEngine::loadAssetIntoWorld(filePath, allocators, idState, mAssetIdToFilepath);
...
bool World::loadScene(const std::string &filePath, bool ignoreSystemsAndCamera)
{
    ...
    PhysicsEngine::loadSceneIntoWorld(filePath, allocators, idState, mSceneIdToFilepath);
...
void World::latentDestroyEntity(const Guid &entityId)
{
    ...
    std::unordered_map<Guid, std::vector<std::pair<Guid, int>>>::const_iterator it =
        mEntityIdToComponentIds.find(entityId);
...
void World::immediateDestroyEntity(const Guid &entityId)
{
    ...
    removeInternalEntityIdFromIndexMap(&mEntityIdToGlobalIndex, &mIdToGlobalIndex, &mIdToType, entityId);
...
void World::immediateDestroyComponent(const Guid &entityId, const Guid &componentId, int componentType)
{
    ...
    {
        swappedComponent = destroyInternalComponent(
            &mTransformAllocator, &mMeshRendererAllocator, &mLineRendererAllocator, &mRigidbodyAllocator,
            &mCameraAllocator, &mLightAllocator, &mSphereColliderAllocator, &mBoxColliderAllocator,
            &mCapsuleColliderAllocator, &mMeshColliderAllocator, componentType, index);
    }
    else
    {
    ...
    if (Component::isInternal(componentType))
    {
        removeInternalComponentIdFromIndexMap(
            ...
            &mMeshColliderIdToGlobalIndex, &mIdToGlobalIndex, &mIdToType, componentId, componentType);

        if (swappedComponent != NULL)
        {
            addInternalComponentIdToIndexMap(
                &mTransformIdToGlobalIndex, &mMeshRendererIdToGlobalIndex, &mLineRendererIdToGlobalIndex,
                &mRigidbodyIdToGlobalIndex, &mCameraIdToGlobalIndex, &mLightIdToGlobalIndex,
                &mSphereColliderIdToGlobalIndex, &mBoxColliderIdToGlobalIndex, &mCapsuleColliderIdToGlobalIndex,
