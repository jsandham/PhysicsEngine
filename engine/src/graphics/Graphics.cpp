...
std::string Graphics::getStandardFragmentShader()
{
    std::string fragmentShader =
        ...
        "    FragColor = vec4(CalcDirLight(material, Normal, viewDir), 1.0f) * texture(material.mainTexture, "
        "TexCoord);\n"
        "#elif defined(SPOTLIGHT)\n"
        "    FragColor = vec4(CalcSpotLight(material, Normal, FragPos, viewDir), 1.0f) * texture(material.mainTexture, "
        "TexCoord);\n"
        "#elif defined(POINTLIGHT)\n"
        "    FragColor = vec4(CalcPointLight(material, Normal, FragPos, viewDir), 1.0f) * "
        "texture(material.mainTexture, TexCoord);\n"
        "#else\n"
        ...
        "    float specularStrength = pow(max(dot(viewDir, reflectDir), 0.0f), material.shininess);\n"
        "    float distance = length(Light.position - fragPos);\n"
        "    float attenuation = 1.0f; // 1.0f / (Light.constant + Light.linear * distance + Light.quadratic * "
        "distance * distance);\n"
