...
void PhysicsSystem::init(World *world)
{
    ...
    //// 		glm::vec4 v1 = model * glm::vec4(mesh->vertices[9*i], mesh->vertices[9*i + 1], mesh->vertices[9*i +
    ///2], 1.0f); / 		glm::vec4 v2 = model * glm::vec4(mesh->vertices[9*i + 3], mesh->vertices[9*i + 4],
    ///mesh->vertices[9*i + 5], 1.0f);
    //// 		glm::vec4 v3 = model * glm::vec4(mesh->vertices[9*i + 6], mesh->vertices[9*i + 7], mesh->vertices[9*i +
    ///8], 1.0f);
    ...
    // std::cout << "number of bounding spheres: " << boundingSpheres.size() << " number of spheres: " << spheres.size()
    // << " number of bounds: " << bounds.size() << " number of triangles: " << triangles.size() << std::endl;

    // Bounds worldBounds = *world->getWorldBounds();

    // glm::ivec3 gridDim = glm::ivec3(worldBounds.size.x / minDiameter, worldBounds.size.y / minDiameter,
    // worldBounds.size.z / minDiameter);

    // gridDim.x = std::min(50, gridDim.x);
    // gridDim.y = std::min(50, gridDim.y);
    // gridDim.z = std::min(50, gridDim.z);

    // std::cout << " min diameter: " << minDiameter << " gridDim.x: " << gridDim.x << " gridDim.y: " << gridDim.y << "
    // gridDim.z: " << gridDim.z << std::endl;

    // for(size_t i = 0; i < boundingSpheres.size(); i++){
    //	Sphere sphere = boundingSpheres[i].sphere;

    //	std::cout << "centre: " << sphere.centre.x << " " << sphere.centre.y << " " << sphere.centre.z << " " <<
    //sphere.radius << " id: " << boundingSpheres[i].id.toString() << std::endl;
    //}
...
void PhysicsSystem::update(Input input, Time time)
{
    ...
    // for(int i = 0; i < world->getNumberOfComponents<SphereCollider>(); i++){
    // 	SphereCollider* collider = world->getComponentByIndex<SphereCollider>(i);
    // 	//std::cout << "collider: " << i << " centre: " << collider->sphere.centre.x << " " << collider->sphere.centre.y
    // << " " << collider->sphere.centre.z << " radius: " << collider->sphere.radius << std::endl;
...
// 	cudaGraphicsGLRegisterBuffer(&(cudaCloths[i].cudaVertexVBO), cloths[i]->vertexVBO.handle,
// cudaGraphicsMapFlagsWriteDiscard); 	cudaGraphicsGLRegisterBuffer(&(cudaCloths[i].cudaNormalVBO),
// cloths[i]->normalVBO.handle, cudaGraphicsMapFlagsWriteDiscard);
...
// 	cudaGraphicsGLRegisterBuffer(&(cudaSolids[i].cudaVertexVBO), solids[i]->vertexVBO.handle,
// cudaGraphicsMapFlagsWriteDiscard); 	cudaGraphicsGLRegisterBuffer(&(cudaSolids[i].cudaNormalVBO),
// solids[i]->normalVBO.handle, cudaGraphicsMapFlagsWriteDiscard);
...
// 		Transform* transform =
// springJoints[i]->getEntity(manager->getEntities())->getComponent<Transform>(manager->getTransforms()); 		Rigidbody*
// rigidbody = springJoints[i]->getEntity(manager->getEntities())->getComponent<Rigidbody>(manager->getRigidbodies());
...
// 		halfVelocity = fac1 * fac2 * halfVelocity - stiffness * timestep * fac2 * (position - targetPosition) + timestep *
// fac2 * (glm::vec3(0.0f, -gravity, 0.0f)); 		position += timestep * halfVelocity;

// 		transform->position = position;
// 		rigidbody->halfVelocity = halfVelocity;

// 		// rigidbody->halfVelocity = rigidbody->velocity + 0.5f * timestep * 100 * (glm::vec3(4.0f, 4.0f, 4.0f) -
// transform->position + glm::vec3(0.0f, -gravity, 0.0f));
// 		// transform->position += timestep * rigidbody->halfVelocity;
// 		// rigidbody->velocity = rigidbody->halfVelocity + 0.5f * timestep * 100 * (glm::vec3(4.0f, 4.0f, 4.0f) -
// transform->position + glm::vec3(0.0f, -gravity, 0.0f));

// 		//Log::Info("iteration: %d velocity %f %f %f", i, rigidbody->velocity.x, rigidbody->velocity.y,
