...
void LineRendererDrawer::render(World *world, EditorProject &project, EditorScene &scene, EditorClipboard &clipboard,
                                Guid id)
{
    if (ImGui::TreeNodeEx("LineRenderer", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        if (ImGui::InputFloat3("Start", glm::value_ptr(start)))
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<glm::vec3>(&lineRenderer->mStart, start, &scene.isDirty));
        }
