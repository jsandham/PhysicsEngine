...
void Hierarchy::update(EditorClipboard& clipboard, bool isOpenedThisFrame)
{
    ...
    if (ImGui::Begin("Hierarchy", &windowActive))
    {
        ...
        if (clipboard.getScene().length() > 0)
        {
            ...
            if (ImGui::BeginPopupContextWindow("RightMouseClickPopup"))
            {
                ...
                if (ImGui::BeginMenu("Create..."))
                {
                    ...
                    {
                        CommandManager::addCommand(new CreateEntityCommand(clipboard.getWorld(), &clipboard.isDirty));
                    }
                    if (ImGui::MenuItem("Camera"))
                    {
                        CommandManager::addCommand(new CreateCameraCommand(clipboard.getWorld(), &clipboard.isDirty));
                    }
                    if (ImGui::MenuItem("Light"))
                    {
                        CommandManager::addCommand(new CreateLightCommand(clipboard.getWorld(), &clipboard.isDirty));
                    }
                    ...
                    if (ImGui::BeginMenu("3D"))
                    {
                        ...
                        {
                            CommandManager::addCommand(new CreateCubeCommand(clipboard.getWorld(), &clipboard.isDirty));
                        }
