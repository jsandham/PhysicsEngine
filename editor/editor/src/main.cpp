...
int serializeScenes(std::string projectDirectory)
{
	...
	for(size_t i = 0; i < sceneFilePaths.size(); i++){
		...
		for(it = objects.begin(); it != objects.end(); it++){
			...
			boids.componentId = Guid(it->first);
			boids.entityId = Guid(it->second["entity"].ToString());
			boids.meshId = Guid(it->second["mesh"].ToString());
			boids.shaderId = Guid(it->second["shader"].ToString());

			boids.numBoids = it->second["numBoids"].ToInt();
			boids.h = (float)it->second["h"].ToFloat();
			boids.bounds.centre.x = (float)it->second["centre"][0].ToFloat();
			boids.bounds.centre.y = (float)it->second["centre"][1].ToFloat();
			boids.bounds.centre.z = (float)it->second["centre"][2].ToFloat();

			boids.bounds.size.x = (float)it->second["size"][0].ToFloat();
			boids.bounds.size.y = (float)it->second["size"][1].ToFloat();
			boids.bounds.size.z = (float)it->second["size"][2].ToFloat();

			std::vector<char> data = boids.serialize();
...
int serializeAssets(std::string projectDirectory)
{
	...
	for(size_t i = 0; i < shaderFilePaths.size(); i++){
		...
		if(AssetLoader::load(filePath, shader)){
			...
			std::vector<char> data = shader.serialize();
	...
	for(size_t i = 0; i < textureFilePaths.size(); i++){
		...
		if(AssetLoader::load(filePath, texture)){
			...
			std::vector<char> data = texture.serialize();
	...
	for(size_t i = 0 ; i < meshFilePaths.size(); i++){
		...
		if(AssetLoader::load(filePath, mesh)){
			...
			std::vector<char> data = mesh.serialize();
