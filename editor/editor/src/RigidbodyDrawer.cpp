...
void RigidbodyDrawer::render(World *world, EditorProject &project, EditorScene &scene, EditorClipboard &clipboard,
                             Guid id)
{
    if (ImGui::TreeNodeEx("Rigidbody", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        if (ImGui::Checkbox("Use Gravity", &useGravity))
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<bool>(&rigidbody->mUseGravity, useGravity, &scene.isDirty));
        }
        ...
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<float>(&rigidbody->mAngularDrag, angularDrag, &scene.isDirty));
        }
