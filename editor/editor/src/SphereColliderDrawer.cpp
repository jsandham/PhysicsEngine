...
void SphereColliderDrawer::render(World *world, EditorProject &project, EditorScene &scene, EditorClipboard &clipboard,
                                  Guid id)
{
        ...
        if (ImGui::TreeNode("Sphere"))
        {
            ...
            if (ImGui::InputFloat3("Centre", glm::value_ptr(centre)))
            {
                CommandManager::addCommand(
                    new ChangePropertyCommand<glm::vec3>(&sphereCollider->mSphere.mCentre, centre, &scene.isDirty));
            }
            if (ImGui::InputFloat("Radius", &radius))
            {
                CommandManager::addCommand(
                    new ChangePropertyCommand<float>(&sphereCollider->mSphere.mRadius, radius, &scene.isDirty));
            }
