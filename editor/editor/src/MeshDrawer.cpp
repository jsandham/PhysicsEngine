...
{
	Mesh* mesh = world->getAssetById<Mesh>(id);

    const int count = 3;
    const char* drawMode[] = { "Color", "Normals", "Tangents"};

    const Guid shaders[] = { world->getColorLitShaderId(), world->getNormalLitShaderId(), world->getTangentLitShaderId() };
    ...
    Shader* shader = world->getAssetById<Shader>(shaders[activeDrawModeIndex]);
