...
void BoxColliderDrawer::render(World* world, EditorProject& project, EditorScene& scene, EditorClipboard& clipboard, Guid id)
{
	if (ImGui::TreeNodeEx("BoxCollider", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (ImGui::TreeNode("Bounds")) {
			glm::vec3 centre = boxCollider->mBounds.mCentre;
			glm::vec3 size = boxCollider->mBounds.mSize;

			if (ImGui::InputFloat3("Centre", glm::value_ptr(centre))) {
				CommandManager::addCommand(new ChangePropertyCommand<glm::vec3>(&boxCollider->mBounds.mCentre, centre, &scene.isDirty));
			}
			if (ImGui::InputFloat3("Size", glm::value_ptr(size))) {
				CommandManager::addCommand(new ChangePropertyCommand<glm::vec3>(&boxCollider->mBounds.mSize, size, &scene.isDirty));
			}
