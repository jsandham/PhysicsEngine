    ...
    if (ImGui::TreeNodeEx("Camera", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        if (ImGui::Combo("Render Path", &renderPath, renderPathNames, 2))
        {
            CommandManager::addCommand(new ChangePropertyCommand<RenderPath>(
                &camera->mRenderPath, static_cast<RenderPath>(renderPath), &scene.isDirty));
        }

        if (ImGui::Combo("Mode", &mode, modeNames, 2))
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<CameraMode>(&camera->mMode, static_cast<CameraMode>(mode), &scene.isDirty));
        }

        if (ImGui::Combo("SSAO", &ssao, ssaoNames, 2))
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<CameraSSAO>(&camera->mSSAO, static_cast<CameraSSAO>(ssao), &scene.isDirty));
        }
        ...
        {
            CommandManager::addCommand(
                new ChangePropertyCommand<Color>(&camera->mBackgroundColor, Color(backgroundColor), &scene.isDirty));
        }
